...
export default function WalkthroughHFW() {
  const { slug: paramSlug } = useParams();
  const slug = paramSlug || 'horizon-forbidden-west';
  ...
  const renderStats = (stats) => {
    ...
    return (
      <div className="flex flex-wrap gap-3 mt-2 mb-4 text-xs text-gray-500 font-medium">
        {stats.artifacts && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><ScrollText size={12} /> 文物: {stats.artifacts}</span>}
        {stats.tradingCards && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><Disc size={12} /> 卡牌: {stats.tradingCards}</span>}
        {stats.coins && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><Disc size={12} /> 硬币: {stats.coins}</span>}
        {stats.journalEntries && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><BookOpen size={12} /> 日志: {stats.journalEntries}</span>}
        {stats.workbenches && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><Hammer size={12} /> 工作台: {stats.workbenches}</span>}
        {stats.safes && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><Lock size={12} /> 保险箱: {stats.safes}</span>}
        {stats.trainingManuals && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><BookOpen size={12} /> 手册: {stats.trainingManuals}</span>}
        {stats.weapons && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><ShieldAlert size={12} /> 武器: {stats.weapons}</span>}
        {stats.collectibles && <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded"><Disc size={12} /> 收集品: {stats.collectibles}</span>}
      </div>
    );
  };
  ...
  if (!game) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#01060b] text-[#e0f7fa]">
        <div className="text-center">
          <h1 className="text-4xl font-bold mb-4 text-[#00f0ff] hfw-font-title">404</h1>
          <p>未找到该游戏的攻略信息</p>
          <a href="/" className="mt-4 inline-block text-[#00f0ff] hover:underline font-medium">返回首页</a>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      className="min-h-screen bg-[#01060b] hfw-hud-bg hfw-font-tech font-sans text-gray-200 selection:bg-black selection:text-[#fdfbf7]"
      style={{
    ...
    >
       ...
       <main className="pt-[100px] pb-20 max-w-[1400px] mx-auto px-4 md:px-6 flex gap-10 lg:gap-14">
          ...
          {/* Article Column */}
          <article className="flex-1 min-w-0 bg-[#030e18] rounded-sm border border-[#00f0ff]/30 shadow-[0_0_30px_rgba(0,240,255,0.15)] p-8 md:p-12 md:pb-16 transition-shadow text-[#e0f7fa] relative z-10"> <div className="hfw-hex-overlay"></div>
             {/* Banner */}
             <div className="w-full aspect-[2/1] rounded-sm overflow-hidden mb-8 shadow-inner bg-[#0c2a46] relative group border border-[#00f0ff]/50 hfw-neon-glow">
               <div className="hfw-focus-ring"></div>
               <img 
             ...
             {/* Title */}
             <h1 className="text-4xl md:text-[45px] font-bold mb-6 leading-[1.2] text-[#00f0ff] tracking-widest hfw-font-title drop-shadow-[0_0_8px_rgba(0,240,255,0.6)]">
               {game.title} 完整图文攻略
               <span className="block text-xl md:text-2xl text-[#ff6a00] mt-3 font-normal hfw-font-tech tracking-widest opacity-90 uppercase">EXPLORE THE FORBIDDEN WEST</span>
             </h1>
             
             {/* Meta */}
             <div className="flex items-center justify-between mb-10 pb-8 border-b border-[#103a60]">
               <div className="flex items-center gap-4">
                 <div className="w-10 h-10 rounded-full overflow-hidden border border-[#103a60] shadow-sm cursor-pointer hover:ring-2 hover:ring-[#00f0ff]/20 transition-all">
                 ...
                 <div className="flex flex-col justify-center">
                   <div className="flex items-center gap-2">
                     <span className="font-bold text-[15px] text-[#e0f7fa] hover:text-[#00f0ff] cursor-pointer transition-colors">{game.author}</span>
                     <span className="px-1.5 py-[1px] bg-[#0c2a46] text-gray-500 text-[10px] font-bold rounded uppercase tracking-wider">官方</span>
                   </div>
             ...
             {/* Content Body */}
             <div className="prose prose-lg max-w-none prose-headings:font-bold prose-headings:text-[#e0f7fa] prose-headings:tracking-tight prose-p:text-[#e0f7fa] prose-p:leading-[1.8] prose-p:mb-6 prose-a:text-[#00f0ff] prose-a:no-underline prose-a:border-b prose-a:border-[#00f0ff]/30 hover:prose-a:border-[#00f0ff] hover:prose-a:bg-[#00f0ff]/10 prose-a:transition-all prose-img:rounded-lg prose-img:shadow-sm prose-blockquote:border-l-4 prose-blockquote:border-[#00f0ff] prose-blockquote:bg-[#0a1f35] prose-blockquote:py-2 prose-blockquote:px-4 prose-blockquote:rounded-r prose-blockquote:not-italic prose-blockquote:text-[#e0f7fa] prose-li:text-[#e0f7fa] prose-strong:text-[#e0f7fa] prose-strong:font-bold">
               ...
               {game.slug === 'the-last-of-us-part-2' ? (
                    ...
                    <div className="mb-12">
                      <div className="flex flex-col md:flex-row gap-6 mb-10 items-stretch">
                        ...
                        <div className="w-full md:w-[320px] lg:w-[360px] flex-shrink-0 flex flex-col gap-4">
                          ...
                          {/* Purchase Card */}
                          <div className="bg-[#061524] rounded-xl shadow-sm border border-[#144b7a] p-5">
                            <h3 className="font-bold text-[#e0f7fa] mb-4 text-sm flex items-center gap-2">
                              <span>🛍️</span> 购买游戏
                            </h3>
                            
                            <div className="flex flex-col gap-4">
                              <div className="relative aspect-[4/5] rounded-xl overflow-hidden shadow-inner border border-[#103a60] bg-[#061524] mb-2">
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex flex-col">
                                  ...
                                  <span className="text-2xl font-bold text-[#00f0ff]">HK$ 398.00</span>
                                </div>
                                <span className="px-2.5 py-1 bg-[#00f0ff]/20 text-[#00f0ff] text-xs font-bold rounded">-30%</span>
                              </div>
                              ...
                              <a 
                                ...
                                className="w-full py-3 bg-[#ff6a00] text-white font-bold rounded-full text-sm hover:bg-[#cc5500] transition-all flex items-center justify-center gap-2 shadow-sm group"
                              >
                         ...
                         <div className="flex-1 flex flex-col">
                           <div className="bg-[#061524] rounded-xl shadow-sm border border-[#144b7a] p-6 flex flex-col h-full">
                             <div className="flex items-center justify-between border-b border-[#103a60] pb-4 mb-4">
                               <h3 className="text-xl font-bold text-[#e0f7fa] flex items-center gap-2">
                                <span className="text-2xl">📋</span> 游戏档案
                              </h3>
                             </div>
                             
                             <div className="flex-1 grid grid-cols-1 content-center gap-1">
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">🏢</span> 开发商
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa]">顽皮狗 (Naughty Dog)</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">📦</span> 发行商
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa]">索尼互动娱乐 (SIE)</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">📅</span> 发售日期
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa]">2020-06-19</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">🎯</span> 游戏类型
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa]">动作冒险 / 生存恐怖</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">🕹️</span> 对应平台
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa]">PS5 / PS4</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">👥</span> 游玩人数
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa]">1人</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">💰</span> 参考价格
                                 </span>
                                 <span className="font-bold text-[#00f0ff] text-base">HK$ 398.00</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">🌐</span> 语言支持
                                 </span>
                                 <span className="font-semibold text-[#e0f7fa] text-right max-w-[50%] leading-tight">简繁中字 / 英文语音</span>
                               </div>
                               <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors last:border-0">
                                 <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm">
                                   <span className="w-5 text-center">⭐</span> 年龄分级
                                 </span>
                                 <div className="flex items-center gap-2">
                                   <span className="px-1.5 py-0.5 border border-[#1a5d96] rounded text-[10px] font-bold text-gray-500">ESRB M</span>
                                   <span className="font-semibold text-[#e0f7fa]">17+ / 18+</span>
                                 </div>
                               </div>
                             </div>
                           </div>
                     ...
                     <h2 className="text-3xl mt-8 mb-6 flex items-center gap-2 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                        <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 w-6 text-center font-normal">#</span>
                        📖 游戏简介
                     </h2>
                     <p className="lead text-[17px] text-[#b2ebf2] leading-relaxed mb-8">
                     ...
                     <h2 className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                    <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                    参考信息
                 </h2>
                 <div className="bg-[#0a1f35] p-6 rounded-lg border border-[#103a60] mb-12">
                   <p className="text-[#80deea] mb-3">
                   ...
                   <a 
                     ...
                     target="_blank" 
                     rel="noopener noreferrer"
                     className="text-[#00f0ff] hover:text-[#42e6f5] underline break-all"
                   >
                 ...
                 <h2 className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                     ...
                     <div className="grid md:grid-cols-3 gap-6 mb-10">
                        <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner">
                          <h4 className="font-bold text-lg mb-2 text-[#00f0ff]">次世代叙事体验</h4>
                          <p className="text-sm text-[#80deea] m-0">
                            通过双主角视角，深入探讨复仇、原谅与人性。电影级的过场动画与无缝衔接的实机游玩，带来沉浸感极强的叙事体验。
                          </p>
                        </div>
                        <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner">
                          <h4 className="font-bold text-lg mb-2 text-[#00f0ff]">进化的战斗系统</h4>
                          <p className="text-sm text-[#80deea] m-0">
                            新增闪避、跳跃与俯卧机制，配合更加智能的AI敌人。玩家需要灵活利用环境潜行或正面对抗，每一场战斗都充满变数。
                          </p>
                        </div>
                        <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner">
                          <h4 className="font-bold text-lg mb-2 text-[#00f0ff]">No Return 模式</h4>
                          <p className="text-sm text-[#80deea] m-0">
                     ...
                     <h2 className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2 flex items-center gap-2">
                        <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                        <span className="text-2xl">🏆</span> 媒体评分
                     </h2>
                     <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        <div className="bg-[#bf1313] text-white p-6 rounded-xl shadow-[0_0_15px_rgba(0,240,255,0.2)] flex flex-col items-center text-center">
                        ...
                        <div className="bg-[#ffcc00] text-[#e0f7fa] p-6 rounded-xl shadow-[0_0_15px_rgba(0,240,255,0.2)] flex flex-col items-center text-center">
                        ...
                        <div className="bg-[#333333] text-white p-6 rounded-xl shadow-[0_0_15px_rgba(0,240,255,0.2)] flex flex-col items-center text-center">
                     ...
                     <div className="space-y-8 mb-10">
                        <figure>
                          <img src={tlou2EllieSeattle} alt="The Last of Us Part II Gameplay" className="w-full rounded-lg shadow-[0_0_15px_rgba(0,240,255,0.2)]" />
                     ...
                     <div className="text-xs text-gray-400 border-t border-[#103a60] pt-4">
                       📌 数据更新于：2026-02-27 | 以上信息汇总自 PlayStation 官网及多家权威媒体，请以官方最新公告为准。
                     </div>
                   </div>
                   
                   <div className="bg-[#061a2b] border border-[#144b7a] rounded-lg p-6 my-10 relative overflow-hidden">
                     <div className="absolute top-0 left-0 w-1 h-full bg-[#00f0ff]"></div>
                     <h4 className="font-bold mb-3 text-[#00f0ff] flex items-center gap-2 text-base">
                        <span>💡</span> 通用技巧
                     </h4>
                     <div className="text-[15px] text-[#e0f7fa] m-0 leading-relaxed space-y-4">
                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 开启“高对比度显示” (High Contrast Display)</strong>
                         <p className="m-0 text-[#80deea] text-sm">建议开启此功能。按下触摸板（原文是 G 键，针对 PC 版或特定设置，主机版通常在辅助功能里设置手势）可以高亮显示物品，让收集品变得非常显眼，极易发现。</p>
                       </div>
                       
                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 如果追求稳妥的通关路线</strong>
                         <p className="m-0 text-[#80deea] text-sm">建议第一周目在“超轻 (Very Light)”难度下完成“永久死亡 (Permadeath)”模式，第二周目再挑战“绝地+ (Grounded+)”难度。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 如果追求速度</strong>
                         <p className="m-0 text-[#80deea] text-sm">可以尝试一次性同时完成两个高难度奖杯（绝地+ 和 永久死亡），然后在“新游戏+ (NG+)”中补齐遗漏的奖杯。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 关于永久死亡 (Permadeath) 模式</strong>
                         <p className="m-0 text-[#80deea] text-sm">建议将设置设定为<strong>“按章节 (Per Chapter)”</strong>，而不是“按幕 (Act)”或“整部游戏 (Whole Game)”。这样容错率最高。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 增强聆听模式 (Enhanced Listen mode)</strong>
                         <p className="m-0 text-[#80deea] text-sm">开启此功能后，聆听模式不仅能侦测敌人，还能为您扫描并“标记 (Ping)”附近的物品，非常实用。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 收集品追踪 (Collectible Tracking)</strong>
                         <p className="m-0 text-[#80deea] text-sm">前往 <strong>选项 &gt; HUD &gt; 通知 &gt; 收集品追踪</strong> 开启此功能。这样当您捡起收集品时会有提示，方便确认哪些已收集，哪些可能错过了。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <strong className="block mb-1 text-[#e0f7fa]">✔ 收集品统计</strong>
                         <p className="m-0 text-[#80deea] text-sm">如果您缺少某个收集品但不知道是在哪一章漏掉的，只需进入<strong>“章节选择”</strong>，系统会显示每一章各类收集品的总数以及您当前的收集进度。您也可以直接通过章节选择进入特定关卡快速补齐。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <h5 className="font-bold text-base mb-2 text-[#e0f7fa]">📦 收集品概览</h5>
                         <p className="mb-2 text-[#80deea] text-sm">《最后生还者 第二部 重制版》的剧情模式中大约有 <strong>286</strong> 个收集品：</p>
                         <ul className="list-disc pl-5 space-y-1 text-[#80deea] text-sm">
                         ...
                         <p className="mt-2 text-[#80deea] text-sm">工程量确实不小，这也是为什么我强烈推荐开启<strong>“高对比度显示”</strong>的原因，找起东西来会轻松很多。</p>
                       </div>

                       <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                         <h5 className="font-bold text-base mb-2 text-[#e0f7fa]">⚠️ 重要提示：关于培训手册</h5>
                         <p className="mb-2 text-[#80deea] text-sm">还有一点需要补充，我很确定<strong>培训手册 (Training Manuals)</strong> 有多个不同的刷新点。这意味着您找到它们的时间点可能与攻略一致，也可能比攻略早或晚。</p>
                         <p className="mb-2 text-[#80deea] text-sm">我知道它们通常会刷新在相同的章节里，但具体位置可能不同。</p>
                         <p className="mb-2 text-[#80deea] text-sm"><strong>关键点：</strong> 如果您按照攻略走到指定位置却没看到手册，请不要慌张，可以去查阅其他视频或指南，看看该章节其他的刷新点在哪里。</p>
                         <p className="m-0 text-[#80deea] text-sm">在我完成这篇指南后，我可能会把其他刷新点也补加上去，但在我们开始游玩时，请务必把这一点记在心里。</p>
                       </div>
                     </div>
                   </div>
                   
                  <h2 id="section-1" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                     一、基础操作与生存技巧
                  </h2>
                  <p>
                  ...
                  <figure className="my-8">
                    <img src={tlou2StealthShot} alt="潜行与侦察" className="w-full rounded-lg shadow-[0_0_15px_rgba(0,240,255,0.2)]" />
                  ...
                  <h3 id="section-1-1" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">1.1 战斗技巧</h3>
                  <p>
                  ...
                  <div className="space-y-12 my-8">
                    {/* Item 1 */}
                    <div>
                       <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">灵活机动</h4>
                       <p className="text-[#80deea] leading-relaxed">
                    ...
                    {/* Item 2 */}
                    <div>
                       <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">闪避是关键</h4>
                       <p className="text-[#80deea] leading-relaxed">
                    ...
                    {/* Item 3 */}
                    <div>
                       <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">利用环境</h4>
                       <p className="text-[#80deea] leading-relaxed">
                    ...
                    {/* Item 4 */}
                    <div>
                       <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">制作与资源</h4>
                       <p className="text-[#80deea] leading-relaxed">
                  ...
                  <h3 id="section-1-2" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">1.2 探索技巧</h3>
                   <div className="space-y-12 my-8">
                     {/* Item 1 */}
                     <div>
                        <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">打碎玻璃</h4>
                        <p className="text-[#80deea] leading-relaxed">
                     ...
                     {/* Item 2 */}
                     <div>
                        <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">检查高处</h4>
                        <p className="text-[#80deea] leading-relaxed">
                     ...
                     {/* Item 3 */}
                     <div>
                        <h4 className="text-lg font-bold mb-2 text-[#e0f7fa]">辅助功能</h4>
                        <p className="text-[#80deea] leading-relaxed">
                  ...
                  <h2 id="section-2" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                     <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                     二、全章节流程攻略
                  </h2>
                  <p>
                  ...
                  <h3 id="section-2-1" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">杰克逊</h3>

                  {/* 详细攻略展开 */}
                  <div className="space-y-6 mt-8 mb-12">
                    {tlou2GuideData.find(c => c.id === 'jackson')?.sections.map((section, idx) => (
                      <div key={idx} className="bg-[#0a1f35] p-5 rounded-lg border border-[#103a60]">
                        <h4 className="text-lg font-bold text-[#e0f7fa] mb-3 flex items-center gap-2">
                        ...
                        <div className="space-y-3">
                          {section.content.map((item, itemIdx) => (
                            <div key={itemIdx} className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                              {item.type === 'collectible' ? (
                                <div className="flex gap-3">
                                  <div className="mt-1 min-w-[16px] flex justify-center">
                                    <div className="w-2 h-2 rounded-full bg-[#00f0ff] mt-1.5 ring-2 ring-red-50"></div>
                                  </div>
                                  <div>
                                    {item.name && <h5 className="text-[#e0f7fa] font-bold text-sm mb-1">{item.name}</h5>}
                                    <p className="text-[#80deea] text-sm leading-relaxed m-0">{item.description}</p>
                                  </div>
                                </div>
                              ) : (
                                 <p className="text-[#80deea] text-sm leading-relaxed m-0 pl-2 border-l-2 border-[#144b7a] ml-1 py-1">{item.text}</p>
                              )}
                            </div>
                          ))}
                        </div>
                  ...
                  <h3 id="section-2-2" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">西雅图 第一天 - 艾莉</h3>
                  ...
                  <h3 id="section-2-3" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">西雅图 第二天 - 艾莉</h3>
                  ...
                  <h3 id="section-2-4" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">西雅图 第三天 - 艾莉</h3>
                  ...
                  <div className="bg-[#0a1f35] p-4 rounded border-l-4 border-gray-400 my-6">
                    <p className="text-sm text-[#80deea] m-0">
                      *后续章节将切换至另一位主角 艾比，体验不同的战斗风格与剧情视角。艾比 的近战能力更强，且拥有独特的武器和技能树。
                    </p>
                  </div>

                  <h3 id="section-2-5" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">公园 - 埃比</h3>
                  ...
                  <h3 id="section-2-6" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">西雅图 第一天 - 埃比</h3>
                  ...
                  <h3 id="section-2-7" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">西雅图 第二天 - 埃比</h3>
                  ...
                  <h3 id="section-2-8" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">西雅图 第三天 - 埃比</h3>
                  ...
                  <h3 id="section-2-9" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">圣塔芭芭拉</h3>
                  ...
                  <h2 id="section-3" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                     <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                     三、保险箱与密码全解
                  </h2>
                  <p>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse bg-[#030e18] rounded-sm overflow-hidden border border-[#00f0ff]/30 shadow-sm border border-[#144b7a]">
                      <thead className="bg-[#0a1f35] text-[#80deea] text-sm uppercase">
                        <tr>
                      ...
                      <tbody className="divide-y divide-gray-100 text-sm">
                        ...
                        <tr><td className="px-4 py-2">超市</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">07-20-13</td><td className="px-4 py-2">入职日期</td></tr>
                        
                        <tr><td className="px-4 py-2 font-bold" colSpan="3">西雅图 第一天 - 艾莉</td></tr>
                        <tr><td className="px-4 py-2">银行金库</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">60-23-06</td><td className="px-4 py-2">抢劫计划图纸</td></tr>
                        <tr><td className="px-4 py-2">西2门</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">04-51</td><td className="px-4 py-2">Checkmate 代码</td></tr>
                        <tr><td className="px-4 py-2">法院</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">86-07-22</td><td className="px-4 py-2">白板上的数字</td></tr>
                        <tr><td className="px-4 py-2">国会山</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">55-01-33</td><td className="px-4 py-2">垃圾箱旁的纸条</td></tr>
                        <tr><td className="px-4 py-2">隧道</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">15243</td><td className="px-4 py-2">苏打水罐上的数字</td></tr>

                        <tr><td className="px-4 py-2 font-bold" colSpan="3">西雅图 第二天 - 艾莉</td></tr>
                        <tr><td className="px-4 py-2">希尔科列斯特</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">30-82-65</td><td className="px-4 py-2">厨房墙上的日历</td></tr>
                        <tr><td className="px-4 py-2">赛拉菲特</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">08-10-83</td><td className="px-4 py-2">结婚纪念日</td></tr>
                        <tr><td className="px-4 py-2">韦斯顿药店</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">38-55-23</td><td className="px-4 py-2">收银台下的纸条</td></tr>

                        <tr><td className="px-4 py-2 font-bold" colSpan="3">西雅图 第三天 - 艾莉</td></tr>
                        <tr><td className="px-4 py-2">水没都市</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">70-12-64</td><td className="px-4 py-2">被困尸体旁的纸条</td></tr>

                        <tr><td className="px-4 py-2 font-bold" colSpan="3">西雅图 第一天 - 艾比</td></tr>
                        <tr><td className="px-4 py-2">步行</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">17-38-07</td><td className="px-4 py-2">彩票号码</td></tr>
                        <tr><td className="px-4 py-2">敌对领土</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">68-96-89</td><td className="px-4 py-2">茉莉花面包店传单</td></tr>
                        <tr><td className="px-4 py-2">海岸</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">90-77-01</td><td className="px-4 py-2">船长室的纸条</td></tr>

                        <tr><td className="px-4 py-2 font-bold" colSpan="3">西雅图 第二天 - 艾比</td></tr>
                        <tr><td className="px-4 py-2">捷径</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">30-23-04</td><td className="px-4 py-2">邻居的公寓号码</td></tr>
                        <tr><td className="px-4 py-2">下降</td><td className="px-4 py-2 font-mono font-bold text-[#00f0ff]">12-18-79</td><td className="px-4 py-2">Wi-Fi 密码</td></tr>
                      </tbody>
                    </table>
                  </div>

                  <h2 id="section-4" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                     <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                     四、No Return 模式
                  </h2>

                  {/* Introduction */}
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    <h4 className="text-lg font-bold text-[#e0f7fa] mb-3 flex items-center gap-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#00f0ff]"></span>
                      模式简介
                    </h4>
                  ...
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    ...
                    <div className="space-y-3">
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                          <h5 className="font-bold text-[#e0f7fa] mb-1">标准挑战 (Standard Runs)</h5>
                          <p className="text-[#80deea] text-sm m-0">从一开始即可游玩。除了你的角色选择外，一切都是随机的。</p>
                      </div>
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                          <h5 className="font-bold text-[#e0f7fa] mb-1">自定义挑战 (Custom Runs)</h5>
                          <p className="text-[#80deea] text-sm m-0">完成一次标准挑战后解锁。允许你自定义敌人类型、Boss 和模组。值得注意的是，自定义挑战不会禁用奖杯，因此非常适合达成特定目标或成就。</p>
                      </div>
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                          <h5 className="font-bold text-[#e0f7fa] mb-1">每日挑战 (Daily Runs)</h5>
                          <p className="text-[#80deea] text-sm m-0">完成九次挑战后解锁。这些预设的挑战对所有玩家都是相同的，并且每天更换。无论输赢，每天只能尝试一次。</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    ...
                    <p className="text-[#80deea] leading-relaxed mb-4">
                  ...
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm text-center hover:border-[#1a5d96] transition-colors overflow-hidden group">
                          ...
                          <h5 className="font-bold text-[#e0f7fa]">艾莉阵营 (Team Ellie)</h5>
                          <p className="text-[#80deea] text-sm">4 名可解锁角色</p>
                      </div>
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm text-center hover:border-[#1a5d96] transition-colors overflow-hidden group">
                          <div className="w-full aspect-video rounded-md overflow-hidden mb-3 shadow-sm">
                          ...
                          <h5 className="font-bold text-[#e0f7fa]">埃比阵营 (Team Abby)</h5>
                          <p className="text-[#80deea] text-sm">4 名可解锁角色</p>
                      </div>
                    </div>
                    
                    <h5 className="font-bold text-[#e0f7fa] mb-2">如何解锁角色：</h5>
                    <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm mb-6 hover:border-[#1a5d96] transition-colors">
                        <p className="text-[#80deea] text-sm m-0 mb-2">解锁角色的方法很简单：<strong>使用该阵营中最新解锁的角色游玩遭遇战。</strong></p>
                        <ul className="list-disc pl-5 space-y-1 text-[#80deea] text-sm">
                    ...
                    <h5 className="font-bold text-[#e0f7fa] mb-2">相关成就：</h5>
                    <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                        <p className="text-[#80deea] text-sm mb-3">这些解锁对于以下成就至关重要：</p>
                        <ul className="space-y-2">
                            <li className="flex items-start gap-2 text-sm text-[#80deea]">
                  ...
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    ...
                    <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                        <p className="text-[#80deea] text-sm leading-relaxed mb-2">
                          在每一轮挑战的最后，你将面对 6 个 Boss 中的一个。你必须击败它们才能解锁新的 Boss。
                        </p>
                        <p className="text-[#80deea] text-sm leading-relaxed m-0">
                  ...
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    ...
                    <div className="space-y-3">
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors flex items-center justify-between">
                          <div>
                            <h5 className="font-bold text-[#e0f7fa] mb-1">坚守模式 (Holdout)</h5>
                            <p className="text-[#80deea] text-sm m-0">完成 5 轮挑战后解锁</p>
                          </div>
                          <span className="text-xs bg-[#00f0ff]/20 text-[#00f0ff] px-2 py-1 rounded border border-red-100 font-medium">成就相关</span>
                      </div>
                      <div className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors flex items-center justify-between">
                          <div>
                            <h5 className="font-bold text-[#e0f7fa] mb-1">占领模式 (Capture)</h5>
                            <p className="text-[#80deea] text-sm m-0">完成 7 轮挑战后解锁</p>
                          </div>
                          <span className="text-xs bg-[#00f0ff]/20 text-[#00f0ff] px-2 py-1 rounded border border-red-100 font-medium">成就相关</span>
                      </div>
                    </div>
                    <p className="text-[#80deea] text-sm mt-4">
                  ...
                  <div className="bg-[#051320] p-5 rounded-sm border-l-4 border-l-[#00f0ff] border-y border-r border-[#0c2a46] shadow-inner mb-8">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        {[
                          ...
                          <div key={i} className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors flex gap-3">
                            <span className="text-[#00f0ff] mt-0.5">🏆</span>
                            <div>
                              <h5 className="font-bold text-[#e0f7fa] text-sm mb-1">{ach.name}</h5>
                              <p className="text-[#80deea] text-xs m-0 leading-relaxed">{ach.desc}</p>
                            </div>
                          </div>
                        ))}
                    </div>
                    <p className="text-[#80deea] text-sm mt-6 leading-relaxed">
                  ...
                  <div className="bg-[#fff1f1] p-6 rounded-lg border border-red-100 mb-10 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-[#00f0ff]/20 rounded-full -mr-12 -mt-12 opacity-50"></div>
                    <h4 className="text-lg font-bold text-red-900 mb-4 flex items-center gap-2 relative z-10">
                      <span className="flex items-center justify-center w-6 h-6 rounded-full bg-[#00f0ff] text-white text-xs">!</span>
                    ...
                    <div className="relative z-10">
                        ...
                        <div className="bg-[#061524]/60 p-4 rounded border border-red-200 shadow-sm">
                  ...
                  <h2 id="section-trophy-list" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                     <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                     五、查看全奖杯列表
                  </h2>
                  <div className="my-8 text-[#e0f7fa]">
                    <p className="mb-6 text-[#e0f7fa] font-medium">
                      需要逐章节图文路线或奖杯筛选列表，请前往专属奖杯页面查看。
                    </p>
                    <Link 
                     to="/the-last-of-us-part-2/trophies"
                     className="block w-full text-center py-4 bg-[#030e18] border border-[#00f0ff] text-[#00f0ff] font-bold uppercase tracking-widest text-sm hover:bg-[#00f0ff] hover:text-[#01060b] transition-all rounded-sm shadow-[0_0_15px_rgba(0,240,255,0.2)] hfw-font-title text-2xl tracking-[4px]"
                   >
                     查看全奖杯列表
                  </Link>
                 </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                   {recommendedGames.map((item, index) => (
                     <Link 
                       ...
                       className="group block bg-[#030e18] rounded-sm overflow-hidden border border-[#00f0ff]/30 shadow-sm hover:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-all border border-[#103a60]"
                     >
                       ...
                       <div className="p-4">
                         <h3 className="font-bold text-[#e0f7fa] group-hover:text-[#00f0ff] transition-colors truncate">
                           {item.title}
                         </h3>
                  ...
                  <hr className="my-12 border-[#103a60]" />
                 </>
                 ...
                 <>
                   ...
                   <div className="mb-12">
                       ...
                       <div className="w-full md:w-[320px] lg:w-[360px] flex-shrink-0 flex flex-col gap-4">
                         <div className="bg-[#061524] rounded-xl shadow-sm border border-[#144b7a] p-5">
                           ...
                           <div className="flex flex-col gap-4">
                             ...
                             <div className="flex items-center justify-between">
                               <div className="flex flex-col">
                                 <span className="text-2xl font-bold text-[#00f0ff]">HK$ 468.00</span>
                               </div>
                             </div>
                             <a 
                    ...
                    <h2 id="section-intro" className="text-2xl mt-8 mb-6 flex items-center gap-2 group cursor-pointer text-[#e0f7fa]">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 w-6 text-center font-normal">#</span>
                       📖 前言与导读
                    </h2>
                    <p className="lead text-[17px] text-[#b2ebf2] leading-relaxed mb-8">
                    ...
                    <h2 id="section-basics" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                       一、基础操作与生存技巧
                    </h2>
                    ...
                    <div className="space-y-8 mt-8 mb-12">
                       {bg3GuideData.find(c => c.id === 'basics')?.sections.map((section, idx) => (
                         <div 
                           key={idx} 
                           className="bg-[#0a1f35] p-6 rounded-lg border border-[#103a60]"
                           style={{
                     ...
                     <h2 id="section-prologue" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                        <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                        二、序章：逃离螺壳舰
                     </h2>
                     <div className="space-y-8 mt-8 mb-12">
                 ...
                 <>
                   ...
                   <div className="mb-12">
                        ...
                        <div className="flex-1 flex flex-col">
                          <div className="bg-[#061524] rounded-xl shadow-sm border border-[#144b7a] p-6 flex flex-col h-full">
                            ...
                            <div className="flex-1 grid grid-cols-1 content-center gap-1">
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">🏢</span> 开发商</span>
                                <span className="font-semibold text-[#e0f7fa]">Guerrilla Games</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                              ...
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">📅</span> 发售日期</span>
                                <span className="font-semibold text-[#e0f7fa]">2022-02-18</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">🎯</span> 游戏类型</span>
                                <span className="font-semibold text-[#e0f7fa]">动作角色扮演 / 开放世界</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                ...
                                <span className="font-semibold text-[#e0f7fa]">PS5 / PS4 / PC</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">💰</span> 参考价格</span>
                                <span className="font-bold text-[#00f0ff] text-base">HK$ 398.00</span>
                    ...
                    <div className="space-y-12 mb-12">
                      ...
                      {/* Core Features */}
                      <div className="bg-[#0a1f35] rounded-xl p-6 border border-[#103a60]">
                         <h3 className="text-xl font-bold text-[#e0f7fa] mb-6 flex items-center gap-2">
                           <span className="text-2xl">🌟</span> 核心特色
                         </h3>
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">🏔️</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">庞大的开放世界</h4>
                             <p className="text-sm text-[#80deea]">探索远未来的美国西部，包含森林、沉没城市和高耸山脉。利用新的跑酷动作和抓钩自由穿梭。</p>
                           </div>
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">🏹</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">激烈的机器战斗</h4>
                             <p className="text-sm text-[#80deea]">利用各种武器、陷阱和策略对抗巨大的机器生物。针对不同机器的弱点制定战术。</p>
                           </div>
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">🏊‍♀️</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">水下探索</h4>
                             <p className="text-sm text-[#80deea]">潜入水下，探索隐藏的秘密和遗迹。全新的潜水面具让你能无限制地在水下活动。</p>
                           </div>
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">🛠️</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">深度RPG系统</h4>
                             <p className="text-sm text-[#80deea]">通过六大技能树（战士、陷阱、生存、渗透、猎人、机器大师）自定义埃洛伊的能力。</p>
                           </div>
                         </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className="bg-[#0a1f35] p-6 rounded-lg border border-[#103a60] mb-6">
                           <p className="text-[#80deea] mb-3">
                             本攻略参考了 Steam 社区的优秀指南，原文链接如下：
                           </p>
                           <a 
                        ...
                        <div className="space-y-8 mb-10">
                           <figure>
                             <img 
                               ...
                               className="w-full rounded-lg shadow-[0_0_15px_rgba(0,240,255,0.2)]" 
                             />
                    ...
                    <div className="bg-[#061a2b] border border-[#144b7a] rounded-lg p-6 my-10 relative overflow-hidden">
                      <div className="absolute top-0 left-0 w-1 h-full bg-[#00f0ff]"></div>
                      <h4 id="section-intro" className="font-bold mb-3 text-[#00f0ff] flex items-center gap-2 text-base">
                         <span>💡</span> 前言与导读
                      </h4>
                      <div className="space-y-6">
                        {horizonForbiddenWestGuideData.find(c => c.id === 'intro')?.sections?.map((section, idx) => (
                           <div key={idx}>
                             <h5 className="font-bold text-[#b2ebf2] mb-2">{section.title}</h5>
                             <div className="space-y-3">
                               {section.content?.map((item, itemIdx) => (
                                 <div key={itemIdx} className="bg-[#061524] p-3 rounded border border-[#144b7a] text-sm text-[#80deea]">
                                   {item.name && <strong className="block mb-1 text-[#e0f7fa]">{item.name}</strong>}
                    ...
                    <h2 id="section-main-quests" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                       一、主线任务流程
                    </h2>
                    ...
                    <div className="space-y-8 mb-12">
                      {horizonForbiddenWestGuideData.find(c => c.id === 'main-quests')?.sections?.map((section, idx) => (
                        <div key={idx} id={`section-main-quests-${['daunt', 'fw', 'sub', 'end'][idx] || idx}`} className="bg-[#0a1f35] p-5 rounded-lg border border-[#103a60]">
                          <h4 className="text-lg font-bold text-[#e0f7fa] mb-3 flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-[#00f0ff]"></span>
                            {section.title}
                          </h4>
                          ...
                          <div className="space-y-3">
                            {section.content.map((item, itemIdx) => (
                              <div key={itemIdx} id={item.id} className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors scroll-mt-[120px]">
                                <h5 className="text-[#e0f7fa] font-bold text-sm mb-1">{item.name}</h5>
                                <p className="text-[#80deea] text-sm leading-relaxed m-0">{item.description}</p>
                                {item.details && (
                                  <div className="mt-4 space-y-4 border-t pt-4 border-[#103a60]">
                                    {item.details.map((detail, dIdx) => {
                                      if (detail.type === 'text') {
                                         // Check for Boss Tip
                                         if (detail.content.includes('Boss战提示') || detail.content.includes('Boss Battle Tip')) {
                                            return (
                                              <div key={dIdx} className="bg-[#00f0ff]/20 border-l-4 border-red-500 p-4 my-4 rounded-r-lg shadow-sm">
                                         ...
                                         if (detail.content.includes('<kbd>')) {
                                            return (
                                              <div key={dIdx} className="bg-slate-100 border-l-4 border-slate-400 p-4 my-3 rounded-r-lg flex items-start gap-3 shadow-sm group hover:bg-slate-200 transition-colors">
                                                 <div className="bg-[#061524] p-1.5 rounded-md shadow-sm border border-slate-200 shrink-0 mt-0.5 group-hover:border-slate-300 transition-colors">
                                         ...
                                         return <p key={dIdx} className="text-[#80deea] text-sm leading-relaxed" dangerouslySetInnerHTML={{__html: detail.content}}></p>;
                                      }
                                      if (detail.type === 'image') {
                                         return (
                                           <div key={dIdx} className="rounded-lg overflow-hidden my-3 shadow-sm border border-[#103a60]">
                                      ...
                                      if (detail.type === 'header') {
                                         ...
                                         if (content.includes('交谈') || content.includes('对话') || content.includes('Talk') || content.includes('Speak')) {
                                           return (
                                             <div key={dIdx} className="bg-[#051320] border border-[#ff6a00]/30 p-3 my-4 rounded-sm flex items-center gap-3 shadow-sm hfw-tribal-glow">
                                               <div className="bg-[#0a1f35] p-1.5 rounded-full shadow-sm text-[#ff6a00] shrink-0">
                                                 <MessageCircle size={18} />
                                               </div>
                                               <h6 className="text-[#e0f7fa] font-bold text-sm m-0 hfw-font-tech">{content}</h6>
                                             </div>
                                           );
                                         }
                                         return <h6 key={dIdx} className="text-[#b2ebf2] font-bold text-sm mt-4 mb-2 pb-1 border-b border-[#103a60]">{content}</h6>;
                                      }
                                      if (detail.type === 'checklist') {
                                         return (
                                           <div key={dIdx} className="bg-[#051320] border border-[#00f0ff]/30 rounded-sm p-4 my-3 hfw-neon-glow">
                                             <ul className="space-y-3">
                                               {detail.items.map((li, liIdx) => (
                                                 <li key={liIdx} className="flex items-start gap-3 text-sm text-[#e0f7fa] hfw-font-tech">
                                                   <CheckCircle2 size={16} className="text-[#00f0ff] flex-shrink-0 mt-0.5" />
                                      ...
                                      if (detail.type === 'list') {
                                         return (
                                           <ul key={dIdx} className="list-disc list-inside text-[#80deea] text-sm space-y-1 pl-2">
                    ...
                    <h2 id="section-side-quests" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                       二、全支线任务列表
                    </h2>
                    ...
                    <div className="space-y-8 mb-12">
                      {horizonForbiddenWestGuideData.find(c => c.id === 'side-quests')?.sections?.map((section, idx) => (
                        <div key={idx} id={['section-side-quests-early', 'section-side-quests-mid', 'section-side-quests-late', 'section-side-quests-arena'][idx]} className="bg-[#0a1f35] p-5 rounded-lg border border-[#103a60]">
                          <h4 className="text-lg font-bold text-[#e0f7fa] mb-3 flex items-center gap-2">
                            <span className="w-1.5 h-1.5 rounded-full bg-[#00f0ff]"></span>
                            {section.title}
                          </h4>
                          ...
                          <div className="space-y-3">
                            {section.content.map((item, itemIdx) => (
                              <div key={itemIdx} id={item.id} className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors scroll-mt-[120px]">
                                {item.name && <h5 className="text-[#e0f7fa] font-bold text-sm mb-1">{item.name}</h5>}
                                <p className="text-[#80deea] text-sm leading-relaxed m-0">{item.text || item.description}</p>
                                {item.details && (
                                  <div className="mt-4 space-y-4 border-t pt-4 border-[#103a60]">
                                    {item.details.map((detail, dIdx) => {
                                      if (detail.type === 'text') {
                    ...
                    <h2 id="section-collectibles" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                       三、全收集品位置
                    </h2>
                    
                    <div className="flex flex-wrap gap-3 mt-2 mb-6 text-xs text-gray-500 font-medium">
                      <span className="flex items-center gap-1 bg-[#0c2a46] px-2 py-1 rounded">
                    ...
                    <div className="space-y-8 mb-12">
                      {['tallnecks', 'cauldrons', 'relic-ruins', 'black-boxes', 'survey-drones', 'vista-points'].map((typeId, typeIdx) => {
                         ...
                         return (
                           <div key={typeId} id={`section-collectibles-${idSuffix}`} className="bg-[#0a1f35] p-5 rounded-lg border border-[#103a60]">
                             <h4 className="text-lg font-bold text-[#e0f7fa] mb-3 flex items-center gap-2">
                               <span className="w-1.5 h-1.5 rounded-full bg-[#00f0ff]"></span>
                               {sectionData.title}
                             </h4>
                             <div className="space-y-3">
                               {sectionData.sections.map((section, secIdx) => (
                                 <div key={secIdx} className="space-y-3">
                                   {section.title && <h5 className="font-bold text-[#b2ebf2] border-l-4 border-[#00f0ff] pl-2">{section.title}</h5>}
                                   {section.content.map((item, itemIdx) => {
                                      if (item.type === 'header') {
                                        ...
                                        if (content.includes('交谈') || content.includes('对话') || content.includes('Talk') || content.includes('Speak')) {
                                          return (
                                            <div key={itemIdx} className="bg-[#051320] border border-[#ff6a00]/30 p-3 my-4 rounded-sm flex items-center gap-3 shadow-sm hfw-tribal-glow">
                                              <div className="bg-[#0a1f35] p-1.5 rounded-full shadow-sm text-[#ff6a00] shrink-0">
                                                <MessageCircle size={18} />
                                              </div>
                                              <h6 className="text-[#e0f7fa] font-bold text-sm m-0 hfw-font-tech">{content}</h6>
                                        ...
                                        return <h6 key={itemIdx} className="text-[#e0f7fa] font-bold text-sm mt-4 mb-1">{content}</h6>;
                                      }
                                      if (item.type === 'text') {
                                        ...
                                        if (content.includes('Boss战提示') || content.includes('Boss Battle Tip')) {
                                           return (
                                             <div key={itemIdx} className="bg-[#00f0ff]/20 border-l-4 border-red-500 p-4 my-4 rounded-r-lg shadow-sm">
                                               <h6 className="flex items-center gap-2 font-bold text-red-700 mb-2 text-sm uppercase tracking-wide">
                                                 <Skull size={18} className="stroke-2" />
                                                 BOSS 战提示
                                               </h6>
                                        ...
                                        return <p key={itemIdx} className="text-[#80deea] text-sm leading-relaxed mb-2" dangerouslySetInnerHTML={{ __html: content }} />;
                                      }
                                      return (
                                     <div key={itemIdx} className="bg-[#061524] p-4 rounded border border-[#144b7a] shadow-sm hover:border-[#1a5d96] transition-colors">
                                       {item.name && <h5 className="text-[#e0f7fa] font-bold text-sm mb-1">{item.name}</h5>}
                    ...
                    <h2 id="section-dlc" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                       四、炙炎海岸 DLC
                    </h2>
                    <div className="space-y-8 mb-12">
                    ...
                    <h2 id="section-trophies" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                       <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                       五、白金奖杯指南
                    </h2>
                    <div className="space-y-6 mt-8 mb-12">
                 ...
                 <>
                   ...
                   <div className="mb-12">
                       ...
                       <div className="w-full md:w-[320px] lg:w-[360px] flex-shrink-0 flex flex-col gap-4">
                         ...
                         <div className="bg-[#061524] rounded-xl shadow-sm border border-[#144b7a] p-5">
                           ...
                           <div className="flex flex-col gap-4">
                             ...
                             <div className="flex items-center justify-between">
                               <div className="flex flex-col">
                                 <span className="text-2xl font-bold text-[#00f0ff]">HK$ 308.00</span>
                               </div>
                             </div>
                        ...
                        <div className="flex-1 flex flex-col">
                          <div className="bg-[#061524] rounded-xl shadow-sm border border-[#144b7a] p-6 flex flex-col h-full">
                            ...
                            <div className="flex-1 grid grid-cols-1 content-center gap-1">
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">🏢</span> 开发商</span>
                                <span className="font-semibold text-[#e0f7fa]">CD PROJEKT RED</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">📦</span> 发行商</span>
                                <span className="font-semibold text-[#e0f7fa]">CD PROJEKT RED</span>
                              ...
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">📅</span> 发售日期</span>
                                <span className="font-semibold text-[#e0f7fa]">2015-05-19 (PS5版 2022-12-14)</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">🎯</span> 游戏类型</span>
                                <span className="font-semibold text-[#e0f7fa]">动作角色扮演 / 开放世界</span>
                              ...
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">🕹️</span> 对应平台</span>
                                <span className="font-semibold text-[#e0f7fa]">PS5 / PS4 / PC / Xbox / Switch</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors border-b border-[#103a60]/50 last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">💰</span> 参考价格</span>
                                <span className="font-bold text-[#00f0ff] text-base">HK$ 308.00</span>
                              </div>
                              <div className="group flex items-center justify-between p-3 rounded-lg hover:bg-[#0a1f35] transition-colors last:border-0">
                                <span className="text-gray-500 font-medium flex items-center gap-2.5 text-sm"><span className="w-5 text-center">⭐</span> 媒体评分</span>
                    ...
                    <div className="space-y-12 mb-12">
                      ...
                      <div className="bg-[#0a1f35] rounded-xl p-6 border border-[#103a60]">
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">🗺️</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">庞大的奇幻开放世界</h4>
                             <p className="text-sm text-[#80deea]">探索饱受战火蹂躏的威伦、繁华的自由之城诺维格瑞以及史凯利杰群岛的严寒地带。</p>
                           </div>
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">⚔️</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">专业的怪物猎人</h4>
                             <p className="text-sm text-[#80deea]">利用各种升级的武器、变种药剂、战斗法印以及陷阱，猎杀各种奇异的怪物。</p>
                           </div>
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">⚖️</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">道德模糊的选择</h4>
                             <p className="text-sm text-[#80deea]">每一个选择都有其后果。你的决定将影响故事的走向、角色的命运甚至整个世界的格局。</p>
                           </div>
                           <div className="bg-[#061524] p-4 rounded-lg shadow-sm border border-[#144b7a]">
                             <div className="text-3xl mb-3">🃏</div>
                             <h4 className="font-bold text-[#e0f7fa] mb-2">昆特牌</h4>
                             <p className="text-sm text-[#80deea]">内置了一款极具深度的卡牌游戏，收集卡牌、构建卡组，在酒馆中与全大陆的对手一决高下。</p>
                           </div>
                         </div>
                      </div>
                    ...
                    {/* Guide Content */}
                    <div className="bg-gradient-to-r from-stone-900 to-slate-800 rounded-xl p-8 my-12 relative overflow-hidden shadow-[0_0_20px_rgba(0,240,255,0.1)] border border-stone-700">
                      ...
                      <div className="relative z-10">
                        <h4 id="section-intro" className="font-bold mb-4 text-red-500 flex items-center gap-3 text-lg tracking-wide uppercase">
                           <span className="bg-[#00f0ff]/200/20 p-2 rounded-lg"><BookOpen size={20} className="text-red-500" /></span> 
                           攻略导读
                        </h4>
                     ...
                     {witcher3GuideData.map((chapter) => (
                       <div key={chapter.id} className="mb-14 relative">
                         ...
                         <div className="flex items-center gap-4 mb-8 group cursor-pointer">
                           <h2 id={`section-${chapter.id}`} className="text-2xl md:text-3xl font-bold text-[#e0f7fa] m-0 flex items-center gap-3">
                         ...
                         <div className="space-y-8 mt-6">
                           {chapter.sections.map((section, idx) => {
                             ...
                             return (
                             <div key={idx} className="bg-[#061524] p-6 rounded-xl border border-[#144b7a] shadow-sm hover:shadow-[0_0_15px_rgba(0,240,255,0.2)] transition-shadow relative overflow-hidden group">
                               {/* Decorative accent */}
                               ...
                               <h4 id={`section-${chapter.id}-${section.id}`} className="text-lg font-bold text-[#e0f7fa] mb-5 flex items-center gap-3 scroll-mt-[100px]">
                               ...
                               <div className="space-y-3 pl-2 border-l-2 border-[#103a60] ml-4">
                                 {section.content.map((item, itemIdx) => {
                                    // Highlight key terms in text
                                    const formattedText = item.text
                                      .replace(/杰洛特/g, '<span class="font-bold text-[#e0f7fa]">杰洛特</span>')
                                      .replace(/希里/g, '<span class="font-bold text-[#e0f7fa]">希里</span>')
                                      .replace(/叶奈法/g, '<span class="font-bold text-[#e0f7fa]">叶奈法</span>')
                                      .replace(/特莉丝/g, '<span class="font-bold text-[#e0f7fa]">特莉丝</span>')
                                    ...
                                    return (
                                     <div key={itemIdx} className="bg-[#0a1f35]/50 p-3 rounded-lg hover:bg-[#0a1f35] transition-colors">
                                       <p 
                                         className="text-[#80deea] text-[14.5px] leading-relaxed m-0"
                     ...
                     <h2 id="section-trophy-list" className="text-3xl mt-16 mb-8 group cursor-pointer text-[#e0f7fa] border-b pb-4 flex items-center gap-2">
                        <span className="text-[#00f0ff] opacity-0 group-hover:opacity-100 transition-opacity -ml-6 absolute w-6 text-center font-normal">#</span>
                        🏆 查看全奖杯列表 (Trophy List)
                     </h2>
                     <div className="my-8 text-[#e0f7fa]">
                 ...
                 <>
                   <p className="lead text-[18px] text-[#e0f7fa] mb-10 font-serif italic pl-2 border-l-4 border-[#144b7a]">
                     {game.description}
                   </p>
               
               <h2 id="section-intro" className="text-3xl mt-12 mb-6 group cursor-pointer text-[#00f0ff] hfw-font-title border-b border-[#00f0ff]/30 pb-2">
                  前言
               </h2>
               <p>
               ...
               <div className="bg-[#061a2b] border border-[#144b7a] rounded-lg p-6 my-10 relative overflow-hidden">
                 ...
                 <p className="text-[15px] text-[#e0f7fa] m-0 leading-relaxed">
               ...
               <h3 id="section-2-1" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">1.1 战斗系统详解</h3>
               <p>
                 战斗是本作的核心体验之一。你需要掌握闪避、格挡与反击的时机。特别是针对精英敌人，盲目的攻击只会让你陷入险境。观察敌人的出招前摇，利用环境优势，才是取胜的关键。
               </p>

               <figure className="my-10">
                 <img src={game.imgUrl} alt="Gameplay" className="w-full rounded-lg shadow-[0_0_15px_rgba(0,240,255,0.2)]" />
               ...
               <h3 id="section-3-1" className="text-xl mt-8 mb-4 font-bold text-[#b2ebf2]">第一章：{game.title} 的序幕</h3>
               <p>
          ...
          {/* Sidebar */}
          <aside className="w-[300px] hidden lg:block shrink-0 space-y-6 sticky top-[100px] h-fit self-start hfw-font-tech relative z-10">
             {/* Author Card */}
             <div className="bg-[#061524] rounded-lg p-6 shadow-[0_0_30px_rgba(0,240,255,0.15)] border border-[#103a60] hover:shadow-[0_0_30px_rgba(0,240,255,0.15)]-hover transition-all group">
               <div className="flex items-center gap-4 mb-4">
                 <div className="w-12 h-12 rounded-full overflow-hidden border border-[#103a60] group-hover:ring-2 group-hover:ring-[#00f0ff]/20 transition-all">
                 ...
                 <div className="flex-1 min-w-0">
                   <h3 className="font-bold text-base text-[#e0f7fa] truncate">{game.author}</h3>
               ...
               <a 
                 ...
                 target="_blank"
                 rel="noopener noreferrer"
                 className="w-full py-2 bg-[#061524] border border-[#00f0ff] text-[#00f0ff] font-bold rounded-full text-sm hover:bg-[#00f0ff] hover:text-[#e0f7fa] transition-all flex items-center justify-center gap-2 shadow-sm"
               >
                  <span>打开官网</span>
               </a>
             </div>
             ...
             {/* TOC */}
              <div className="bg-[#061524] rounded-lg p-6 shadow-[0_0_30px_rgba(0,240,255,0.15)] border border-[#103a60] sticky top-4 flex flex-col max-h-[calc(100vh-2rem)]">
                <h3 className="font-bold text-[#e0f7fa] mb-4 pl-1 text-[15px] flex items-center justify-between shrink-0">
                  目录
                ...
                <nav className="space-y-1 relative pr-2 flex-1 overflow-y-auto custom-scrollbar">
                 <div className="absolute left-[7px] top-2 bottom-2 w-[2px] bg-[#0c2a46]"></div>
                       ...
                       <div className="flex items-center group relative">
                         <a 
                           ...
                           className={`
                             ...
                             ${(isParentActive || isChildActive)
                               ? 'text-[#00f0ff] font-bold bg-[#00f0ff]/20/80 border-[#00f0ff] shadow-sm' 
                               : 'text-gray-500 hover:text-[#00f0ff] hover:bg-[#0a1f35] border-transparent hover:border-[#144b7a] font-medium'}
                           `}
                         >
                         ...
                         {hasChildren && (
                           <button
                             ...
                             className={`absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-md transition-colors z-20 
                               ${(isParentActive || isChildActive) 
                                 ? 'text-[#00f0ff] hover:bg-red-100' 
                                 : 'text-gray-400 hover:text-[#80deea] hover:bg-[#0c2a46]'}`}
                           >
                             ...
                             return (
                               <div key={child.id}>
                                       ...
                                       <a 
                                         ...
                                         className={`
                                           ...
                                           ${(isCurrentChildActive || isGrandChildActive)
                                             ? 'text-[#00f0ff] font-bold bg-[#00f0ff]/10 border-[#00f0ff]' 
                                             : 'text-gray-500 hover:text-[#00f0ff] hover:bg-[#0a1f35] border-transparent hover:border-[#144b7a]'}
                                         `}
                                       >
                                         {child.title}
                                       </a>
                                   ...
                                   {child.children && child.children.length > 0 && isChildExpanded && (
                                       <div className="space-y-1 mt-1 ml-4 border-l border-[#103a60]">
                                               ...
                                               return (
                                                   <a
                                                       ...
                                                       className={`
                                                           ...
                                                           ${isGrandChildCurrent
                                                               ? 'text-[#00f0ff] font-bold border-[#00f0ff] bg-[#00f0ff]/20/30'
                                                               : 'text-gray-500 hover:text-[#00f0ff] border-transparent hover:border-[#144b7a]'}
                                                       `}
                                                   >
                                                       {grandChild.title}
                                                   </a>
       ...
       <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-[400px] bg-[#061524]/95 backdrop-blur-md border border-[#144b7a] shadow-[0_8px_30px_rgba(0,0,0,0.12)] rounded-full z-40 lg:hidden px-6 py-3 flex items-center justify-between">
         <div className="flex items-center gap-8">
           <button 
             onClick={() => setIsShareOpen(true)}
             className="flex flex-col items-center justify-center text-[#80deea] hover:text-[#e0f7fa] transition-colors"
           >
             <Share2 size={22} />
           </button>
           <button 
           ...
           >
             <div className="relative">
               <Heart 
                 size={22} 
                 className={`transition-colors duration-300 ${isLiked ? 'fill-red-500 text-red-500' : 'text-[#80deea] hover:text-[#e0f7fa]'}`}
               />
         ...
         <button 
           ...
           className="flex items-center justify-center text-[#80deea] hover:text-[#e0f7fa] transition-colors"
         >
       ...
       {isTocOpen && (
         <div className="fixed inset-0 z-50 lg:hidden flex flex-col justify-end">
           ...
           {/* Sheet */}
           <div className="relative bg-[#061524] w-full max-h-[80vh] rounded-t-2xl shadow-xl flex flex-col animate-slide-up">
             {/* Handle */}
             ...
             {/* Header */}
             <div className="px-6 pb-4 border-b border-[#103a60] flex items-center justify-between">
               <h3 className="font-bold text-[#e0f7fa] text-lg flex items-center gap-2">
                 <BookOpen size={18} className="text-[#00f0ff]" />
                 目录
               </h3>
               <div className="flex items-center gap-3">
                 <span className="text-xs font-bold text-white bg-[#00f0ff] px-2 py-0.5 rounded-full shadow-sm">{readingProgress}%</span>
                 <button onClick={() => setIsTocOpen(false)} className="text-gray-400 hover:text-[#80deea]">
                   <X size={20} />
                 </button>
               </div>
             </div>
       ...
       {isShareOpen && (
         <div className="fixed inset-0 z-50 lg:hidden flex flex-col justify-end">
           ...
           {/* Sheet */}
           <div className="relative bg-[#0a1f35] w-full rounded-t-2xl shadow-xl flex flex-col animate-slide-up pb-safe">
             {/* Header Card */}
             <div className="bg-[#061524] rounded-t-2xl p-4 shadow-sm relative">
               <button 
                 onClick={() => setIsShareOpen(false)} 
                 className="absolute top-4 right-4 text-gray-400 hover:text-[#80deea]"
               >
                 <X size={24} />
               </button>
               
               <div className="flex items-center gap-3 pr-10">
                 <div className="w-10 h-10 rounded-lg overflow-hidden border border-[#103a60] shadow-sm shrink-0">
                   <img src={game?.imgUrl || tlou2Ps5Cover} alt={`${game?.title || 'Game'} Cover`} className="w-full h-full object-cover" />
                 </div>
                 <div className="flex-1 min-w-0">
                   <h3 className="font-bold text-[#e0f7fa] text-[15px] truncate">
             ...
             <div className="p-6">
               <div className="grid grid-cols-4 gap-y-6 gap-x-4 mb-6">
                 ...
                 >
                   <div className="w-14 h-14 rounded-2xl bg-[#061524] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform">
                   ...
                   <span className="text-[11px] text-[#80deea]">微信</span>
                 </button>
                 <button 
                   onClick={handleWechatShare}
                   className="flex flex-col items-center gap-2 group"
                 >
                   ...
                   <span className="text-[11px] text-[#80deea]">朋友圈</span>
                 </button>
                 <button 
                 ...
                 >
                   ...
                   <span className="text-[11px] text-[#80deea]">微博</span>
                 </button>
                 ...
                 <button 
                   ...
                   className="flex flex-col items-center gap-2 group"
                 >
                   <div className="w-14 h-14 rounded-2xl bg-[#061524] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform text-[#b2ebf2]">
                   ...
                   <span className="text-[11px] text-[#80deea]">保存图片</span>
                 </button>
                 <button 
                   onClick={handleCopyLink}
                   className="flex flex-col items-center gap-2 group"
                 >
                   <div className="w-14 h-14 rounded-2xl bg-[#061524] flex items-center justify-center shadow-sm group-hover:scale-105 transition-transform text-[#b2ebf2] relative">
                     {copySuccess ? (
                   ...
                   <span className="text-[11px] text-[#80deea]">复制链接</span>
                 </button>
                 <button 
                 ...
                 >
                   ...
                   <span className="text-[11px] text-[#80deea]">系统分享</span>
                 </button>
               </div>
             </div>
           </div>
